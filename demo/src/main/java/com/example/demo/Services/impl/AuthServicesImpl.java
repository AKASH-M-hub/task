/// generate thefull code for the auth services class in the package com.example.demo.Services with methods for register, login and logout. The register method should take a User object and save it to the database, the login method should take a username and password and return a JWT token if the credentials are valid, and the logout method should invalidate the JWT token.
/// You can use the following code as a starting point for the AuthServices class:
/// Make sure to add the necessary annotations for the service class and also inject the UserRepository to perform the database operations.
/// Also create the necessary DTOs for the auth response in the package com.example.demo.DTO.response with fields token of type String.
